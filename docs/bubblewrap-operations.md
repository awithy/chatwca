...
The hardened `chatwca.service` uses `PrivateUsers=no`, `RestrictNamespaces=no`, `NoNewPrivileges=yes`, `KillMode=control-group`, process-wide `TasksMax=512`, `LimitNOFILE=8192`, and `TimeoutStopSec=310s`. The profile passed with `NoNewPrivileges` set both through `setpriv` and a systemd-managed transient service. These settings do not prevent the tested namespace creation on this host.
