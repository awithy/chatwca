...
On command timeout or conversation abort, ChatWCA terminates the entire Bubblewrap worker rather than trusting process-group cleanup. The kernel then removes every process in the PID namespace. Namespace invalidation is an explicit milestone: once it completes, the active tool rejects without awaiting the rest of the restart transition. This ordering is required because that transition aborts Pi and waits for Pi to become idle, while Pi cannot become idle until the active tool settles. The full abort caller—and timeout-driven `agent_end` publication—continues to await Pi settlement and a successful replacement handshake. New prompts remain blocked until the controller is healthy. Ephemeral home and temporary state are intentionally lost after abort.
