...
- Network access from the **parent** to any remote model provider and, when `BRAVE_SEARCH_API_KEY` is configured, `api.search.brave.com`. `PI_OFFLINE` intentionally disables Pi model network access but does not disable the separately configured Brave tool.
