...
You can use **Workspace Info** to inspect a workspace's path, availability, session-storage policy, stored/effective security and network policy, stored/effective destination-policy set, normalized grants, and disclosure warnings. Add/edit opens a keyboard-contained responsive modal; Escape closes only while idle and focus returns to the exact trigger. You can rename a workspace at any time. Changing its path, security profile, network type, or destination-policy set—or removing it—requires closing all live conversations in that workspace first. The session-storage policy cannot be edited. Removing a workspace unregisters only its ChatWCA metadata: the directory, its contents, and all Pi JSONL sessions are retained.
...
| `CHATWCA_NETWORK_ALLOWED_DOMAINS` | `[]` | Global domain ceiling as a JSON string array of exact, `*.` subdomain-only, or `**.` apex-plus-subdomain patterns. Browser clients cannot modify it. Non-empty in optional mode. |
...
- `./data/chatwca.sqlite` contains only registered workspace IDs, names, canonical directory paths, immutable session-storage policies, stored security/network policy and destination-policy-set IDs, and timestamps. Schema v5 migration gives every prior workspace the set ID `default`; it does not copy destination rules into SQLite. `CHATWCA_DATA_DIR` changes its parent directory. The repository's `/data/` rule ignores the database and its `-wal`, `-shm`, and journal sidecars.
