...
A managed-egress policy decision is recorded as a bounded audit event with workspace/conversation and selected-set identity, protocol, normalized host, port, decision, and stable reason. Blocked decisions may emit rate-limited `network.blocked` events. The browser shows these events in a dismissible conversation banner; dismissal clears the local projection only, and a later blocked event shows the banner again. Audit and browser payloads exclude URL paths, queries, headers, bodies, TLS data, credentials, command output, resolved addresses, host paths, and private diagnostics.
...
At phone widths (720 px and below), the full conversation header and persistent Conversations/Jobs rail are removed to preserve vertical chat space. A compact application bar shows the truncated conversation title, run status, security profile, connection state, workspace-drawer trigger, and a contextual overflow trigger. The workspace drawer also contains the Conversations/Jobs section switcher. The overflow menu retains model, context, workspace, and security facts plus New, Rename, Close, and Delete actions; destructive and unavailable actions preserve their desktop confirmation and locking rules. The Jobs page uses the same compact-bar and drawer navigation pattern.
...
- at phone widths, the full conversation header and bottom section navigation are replaced by a compact contextual app bar, drawer-based section navigation, and an overflow action menu without losing security/status visibility or conversation actions;
